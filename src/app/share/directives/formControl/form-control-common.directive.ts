...
@Directive({ selector: "[appFormControlCommon]" })
export class FormControlCommonDirective implements OnInit, AfterViewInit {
  ...
  public ngOnInit(): void {
    ...
    if (Boolean(this.formControlName) && Boolean(this.parentFormContainer)) {
      this.formControl = (this.parentFormContainer.control as FormGroup)
        .controls[this.formControlName] as FormControl;
      if (this.formControl === undefined) {
        throw new Error(
          `Form control ${this.formControlName} is not registered in form group`,
        );
      }
    } else {
